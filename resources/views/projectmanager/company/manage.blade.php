          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Total Company : {{\App\Models\User::count()}} </h3>
                  ...
                  @foreach($companys as $item)
                    <tr>
                        ...
                        <td>
                            ...
                            <form class="float-left px-2" action="{{ route('company_account.destroy',$item->id) }}" method="POST">
                                @csrf 
                                @method('delete')
                        ...
                        <div class="modal fade" id="edit_{{$item->id}}">
                                ...
                                <div class="modal-content bg-secondary">
                                    ...
                                    <form class="add-contact-form" method="post" action="{{ route('company_account.update',$item->id) }}" enctype="multipart/form-data">
                                        @csrf
                                        @method('patch')
                                        ...
                                        <button type="submit" class="btn btn-outline-light">Update Company</button>
                                        </div>
                                    </form>  
                                </div>
                                <!-- /.modal-content -->
  ...
  <div class="modal fade" id="branch-info">
        <div class="modal-dialog">
          <div class="modal-content bg-secondary">
            ...
            <form class="add-contact-form" method="post" action="{{ route('company_account.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                ...
                <div class="modal-footer justify-content-between">
                <button type="button" class="btn btn-outline-light" data-dismiss="modal">Close</button>
                <button type="submit" class="btn btn-outline-light">Save Company Account</button>
                </div>
            </form>  
          </div>
          <!-- /.modal-content -->
...
$('input[name=toogle]').change(function(){
   ...
   //alert(id);
   $.ajax({
       url:"{{ route('company_account.status')}}",
       type:"POST",
       data:{
           _token:'{{csrf_token()}}',
           mode:mode,
...
$('input[name=vendor]').change(function(){
   ...
   //alert(id);
   $.ajax({
       url:"{{ route('company_vendor.status')}}",
       type:"POST",
       data:{
           _token:'{{csrf_token()}}',
           mode:mode,
